...
namespace TGCore.HarmonyPatches
{
    ...
    class ArmorProjectilePatch
    {
        ...
        public static bool Prefix(ProjectileHit __instance, RaycastHit sentHit, float multiplier, ref MoveTransform ___move, ref RaycastTrail ___trail, ref TeamHolder ___teamHolder)
        {
            ...
            var teamHolder = __instance.GetComponentInParent<TeamHolder>();

            if (!__instance.GetComponent<ProjectileHoming>() && sentHit.transform && armoredUnit &&
                armoredUnit.armorActive && sentHit.rigidbody &&
                armoredUnit.blockPower > __instance.blockPoweredNeeded &&
                (!teamHolder || teamHolder.team != armoredUnit.GetComponent<Unit>().Team))
            {

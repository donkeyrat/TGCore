...
public class SecretUnlockPiece : GameStateListener
{
	...
	public List<string> otherKeysToCheck;
	...
	[Header("Final unlock")]
	public string finalUnlockKey;
	public string finalUnlockDescription;
	public Sprite finalUnlockIcon;
	...
	private IEnumerator UnlockSecret()
	{
		...
		var numberOfKeysUnlocked = otherKeysToCheck.Sum(x =>
			ServiceLocator.GetService<ISaveLoaderService>().HasUnlockedSecret(x) ? 1 : 0) + 1;
		
		ServiceLocator.GetService<ModalPanel>().OpenUnlockPanel(numberOfKeysUnlocked + "/" + (otherKeysToCheck.Count+1) + " " + secretDescription, secretIcon);

		if (numberOfKeysUnlocked == otherKeysToCheck.Count + 1 && !string.IsNullOrEmpty(finalUnlockKey))
		{
			ServiceLocator.GetService<ISaveLoaderService>().UnlockSecret(finalUnlockKey);
			ServiceLocator.GetService<ModalPanel>().OpenUnlockPanel(finalUnlockDescription, finalUnlockIcon);
		}
	}
